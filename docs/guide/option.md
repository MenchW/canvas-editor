...
interface IEditorOption {
  mode?: EditorMode // 编辑器模式：编辑、清洁（不显示视觉辅助元素。如：分页符）、只读、表单（仅控件内可编辑）、打印（不显示辅助元素、未书写控件及前后括号）、设计模式（不可删除、只读等配置不控制）、涂鸦模式（屏蔽选区、允许长按移动绘制线条）、留痕模式、无痕编辑模式（preview_edit：隐藏控件辅助元素与占位符，可自由修改文本内容）。默认：编辑
