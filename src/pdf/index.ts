...
import { IElementMetrics, IElementPosition } from '../editor/interface/Element'
import { IRow, IRowElement } from '../editor/interface/Row'
...
export class Pdf {
  ...
  private _computeRowList(innerWidth: number, elementList: IElement[]) {
    const { defaultSize, defaultRowMargin, defaultBasicRowMarginHeight } = this.editorOptions
    const tdPadding = (this.editorOptions as any).tdPadding ?? 5
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if (element.type === ElementType.IMAGE) {
        ...
        metrics.boundingBoxAscent = 0
        // 表格分页处理(拆分表格)
        const margins = this.editorOptions.margins
