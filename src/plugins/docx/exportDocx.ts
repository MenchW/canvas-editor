...
function inferImageType(data: string): 'png' | 'jpg' | 'gif' | 'bmp' {
  if (typeof data !== 'string') return 'png'
...
function getImageData(url: string): Uint8Array | null {
  if (!url || typeof url !== 'string') return null
  try {
    if (url.startsWith('data:image/')) {
      const base64Str = url.replace(/^data:image\/[^;]+;base64,/, '').replace(/\s/g, '').trim()
    ...
    console.warn('[exportDocx] 图像 Base64 解析失败，已兜底:', e)
  }
  return null
}
...
function convertElementToParagraphChild(element: IElement): ParagraphChild {
  if (element.type === ElementType.IMAGE) {
    const bytes = getImageData(element.value)
    if (!bytes) {
      return new TextRun({ text: '[图片]' })
    }
    return new ImageRun({
